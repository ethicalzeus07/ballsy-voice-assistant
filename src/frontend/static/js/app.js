...
function initWebSocket() {
    ...
    appState.webSocket = new WebSocket(`${WS_BASE_URL}/ws/voice/${appState.currentUserId}`);
...
async function loadConversationHistory() {
    try {
        const response = await fetch(`${API_BASE_URL}/api/history/${appState.currentUserId}?limit=10`);
...
async function loadSettings() {
    try {
        const response = await fetch(`${API_BASE_URL}/api/settings/${appState.currentUserId}`);
